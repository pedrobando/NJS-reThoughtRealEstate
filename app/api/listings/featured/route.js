...
export async function GET() {
  try {
    const response = await fetch(process.env.HOMEJUNCTION_RE_LITINGS_URI, {
      method: "GET",
      headers: {
    ...
    if (data.result?.responses) {
      // Check for invalid featured parameter
      const invalidFeatured = data.result.responses.some((response) =>
        response.result?.invalid?.includes("featured"),
      );

      if (invalidFeatured) {
        return NextResponse.json(
          { error: "Invalid 'featured' parameter in the request" },
          { status: 422 },
        );
      }
      ...
      if (allListingsEmpty) {
        ...
        return NextResponse.json({
          success: true,
          result: { listings: [] },
          message: "No featured listings found",
        });
      }
    }
